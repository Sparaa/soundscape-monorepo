...
interface VisualFrame {
  ...
  beat: { phase: number; index: number; bar: number; bpm: number; hit: number; punch: number };
        ...
        // punch 0..1: bass transient envelope — positive spectral flux 30–150 Hz against the track's own running
        // floor, no refractory period, ~100 ms decay; the center of the scope pulses with it (kicks, 808s, bass plucks;
        // snares, hats and vocals are above the cutoff)

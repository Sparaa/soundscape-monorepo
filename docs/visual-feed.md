...
interface VisualFrame {
  ...
  beat: { phase: number; index: number; bar: number; bpm: number; hit: number; punch: number };
        ...
        // punch 0..1: bass transient envelope — positive spectral flux ~50–150 Hz against the track's own running
        // floor, gated by the band's absolute level (nothing below ~-65 dBFS, full above ~-47 dBFS), no refractory
        // period, ~100 ms decay; the center of the scope pulses with it (audible kicks, 808s, bass plucks; sub-bass,
        // snares, hats and vocals are outside the band, faint transients are gated out)
